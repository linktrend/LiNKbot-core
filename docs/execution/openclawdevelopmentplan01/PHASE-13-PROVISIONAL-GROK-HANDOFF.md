...
| 5   | Fixture owner sign-off / aggregate hashes missing                     | Sign-off process with exact aggregates; status **`AWAITING_OWNER_COUNTERSIGN`** (Principal must route to Brain/Skills owners)  | `FIXTURE-OWNER-SIGNOFF.md`                                                                             |
| 6   | Crabbox/Testbox heavy proof unclear or overclaimable                  | Local `crabbox` binary **missing** / sanity failed; **trusted local fallback** used; **draft PR #38** opened for exact-head CI | PR https://github.com/linktrend/openclaw_prime/pull/38 ; HEAD `17508f539d7`                            |
