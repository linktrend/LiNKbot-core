...
export function parseInstantMs(iso: string): number | null {
  ...
  const mm = Number(m[5]);
  const ss = Number(m[6]);
  if (!isValidCalendarDate(`${m[1]}-${m[2]}-${m[3]}`)) {
    return null;
  }
...
function isValidWindow(w: WaveWindow, nowMs: number): boolean {
  if (!isValidCalendarDate(w.cycleDate)) {
    return false;
  }
...
export function planOfflineReconcile(input: OfflineReconcilePlanInput): OfflineReconcilePlan {
  ...
  if (nowMs === null) {
    return {
      ...
      failureIdsToReconcile: [...new Set(input.unresolvedFailureIds)].toSorted(),
      allowContinuousPolling: false,
      allowBlindReplay: false,
  ...
  const cycles = [...byCycle.keys()].toSorted();
  const latestCycle = cycles.at(-1) ?? null;
  const windowsToConsider = latestCycle
    ? (byCycle.get(latestCycle) ?? [])
        .slice()
        .toSorted((a, b) => WAVE_RANK[a.wave] - WAVE_RANK[b.wave])
    : [];
