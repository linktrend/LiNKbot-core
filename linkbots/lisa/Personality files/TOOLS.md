...
## Model Stack (OCP-W10-LISA-RELEASE approved — workshop contract)
...
| `minimax`   | `minimax/MiniMax-M3`                      | **Image/PDF** (`imageModel.primary`) + vision path                    |
...
- **Image/PDF:** `minimax/MiniMax-M3` via `agents.defaults.imageModel.primary`
...
2. **Images / PDFs** → `minimax/MiniMax-M3` imageModel path (not OpenRouter M3 unless separately proven).
...
6. **Sonnet** → manual selection only when Carlos says the name or equivalent.
7. **Nemotron** → eval runs only; log to `memory/evals/`; never enable paid spend from automation.
