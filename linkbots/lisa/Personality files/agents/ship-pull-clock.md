...
One line only for the wave, e.g. `Ship A: Clear` or `Pull B: Issues`. Mirror shapes in `agents/pipeline-status.md`. Prefer also keeping the personality-mirror copy in sync when editing in-repo: `memory/pipeline-status.md`.
...
Replace `WAVE` with `Ship A` or `Ship B`.
...
Replace `WAVE` with `Pull A` or `Pull B`.
...
Create four jobs named `lisa-ship-a`, `lisa-pull-a`, `lisa-ship-b`, `lisa-pull-b` with the exprs above, isolated session, `agentId: lisa-cron`, announce → Telegram `1123023078`, message pointing at this procedure (e.g. “Run agents/ship-pull-clock.md for Ship A”). Prefer matching flags used by `lisa-morning-digest`.
