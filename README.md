...
> Production status: the pre-configuration engineering baseline is complete. See [the production-readiness index](https://docs.openclaw.ai/production-readiness) for remaining configuration, staging, deployment, and operational acceptance work.
...
2. [`docs/agent-briefing.md`](docs/agent-briefing.md) — current VPS source-of-truth and operating boundaries
3. [`docs/agent-coordination.md`](docs/agent-coordination.md)
4. [`docs/current-status.md`](docs/current-status.md) — current Lisa/OpenClaw/Brain/Skills/Platform baseline
...
This repo may have Orchestrator, Lisa, and Feature agents working through Cursor, Codex, and ChatGPT on different machines and execution surfaces. Every agent creates its own session record; only Orchestrators maintain the shared dashboard. Use one branch per task and create a dated handoff for material work. The current VPS Lisa release is integrated with LiNKbrain and LiNKskills, and the controlled Librarian path has reached canonical knowledge. Verify Git, deployment, health, and behavioral claims separately; never overwrite another session's work or include secrets, tokens, private memory, emails/messages, or unrelated private data in coordination records. Do not treat `linkbots/` workshop files as live mutable VPS state.
