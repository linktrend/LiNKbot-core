...
const ANDROID_TOOL_DISPLAY_GENERATED_RE =
  /^apps\/android\/app\/src\/main\/(?:java\/ai\/openclaw\/app\/i18n\/NativeStringResources\.kt|res\/values-[^/]+\/strings\.xml)$/;
const TOOL_DISPLAY_SNAPSHOT_PATH =
...
export function assertNativeGeneratedArtifactsIsolated(changedPaths, branchName = "") {
  ...
  // tool-display.json is a deterministic snapshot consumed by the Android
  // resource generator. Permit that exact cascade to travel with its source
  // change because strict Android parity verifies every generated byte. Other
  // native translations and Apple artifacts remain isolated.
  if (
    generatedPaths.every((filePath) => ANDROID_TOOL_DISPLAY_GENERATED_RE.test(filePath)) &&
    generatedCompanionPaths.includes(TOOL_DISPLAY_SNAPSHOT_PATH)
  ) {
    return;
  }
